...
if transitionProbabilitiesImplemented
    ...
    % TODO: Question b)
    myP = ComputeTransitionProbabilities(stateSpace, map);
end
toc
%% Compute stage costs
if stageCostsImplemented 
    ...
    % TODO: Question c)
    myG = ComputeStageCosts(stateSpace, map, myP);
end
toc
...
if valueIterationImplemented
    ...
    % TODO: Question d)
    [ J_opt_vi, u_opt_ind_vi ] = ValueIteration(myP, myG);
...
if policyIterationImplemented
    ...
    % TODO: Question d)
    [ J_opt_pi, u_opt_ind_pi ] = PolicyIteration(myP, myG);
...
if linearProgrammingImplemented
    ...
    % TODO: Question d)
    [ J_opt_lp, u_opt_ind_lp ] = LinearProgramming(myP, myG);
...
%     end
% end
% disp([num2str(m), ' correct data out of ', num2str(476*476*5), ' in transition probability matrix']);
% disp(476*476*5);
% 
% m =0;
% G_difference = zeros (476,5);
% for i = 1:476
% %     for j = 1:476
%         
%         for k =1:5
%             if G(i,k) == myG(i,k) || abs(G(i,k)-myG(i,k)) < 1e-15
%                 m = m +1;
%             else
%                 G_difference(i,k) = myG(i,k) - G(i,k);
%             end
% %         end
%     end
% end
% disp(m/476/5);
% disp(sum(G_difference(:)));
toc